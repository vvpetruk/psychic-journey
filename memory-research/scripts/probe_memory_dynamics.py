...
def install_memory_probe(backend: torch.nn.Module) -> list[dict[str, Any]]:
    ...
    for layer_idx, block in enumerate(backend.blocks):
        ...
        def wrapped_forward(x, state=None, return_state=True, _mem=mem, _layer_idx=layer_idx, _orig=original_forward):
            ...
            alpha = _mem.gate_decay(x_mean).mean()
            ...
            grad_norm = sum(tensor_norm(g) for g in grads)

            output, new_state = _orig(x, state=state, return_state=return_state)

            post_weight_norm = 0.0
            post_momentum_norm = 0.0
...
def run_case(case: dict[str, Any], token_stream: torch.Tensor, vocab_size: int) -> dict[str, Any]:
    ...
    traces = install_memory_probe(backend)
...
def main() -> None:
    ...
    cases = [
        {'name': 'baseline', 'memory_lr': 0.1, 'memory_momentum': 0.9, 'memory_decay': 0.01, 'steps': 8},
        {'name': 'reduced_lr', 'memory_lr': 0.01, 'memory_momentum': 0.9, 'memory_decay': 0.01, 'steps': 8},
        {'name': 'tiny_lr', 'memory_lr': 0.001, 'memory_momentum': 0.9, 'memory_decay': 0.01, 'steps': 8},
        {'name': 'low_lr_low_momentum', 'memory_lr': 0.001, 'memory_momentum': 0.1, 'memory_decay': 0.01, 'steps': 8},
        {'name': 'low_lr_higher_decay', 'memory_lr': 0.001, 'memory_momentum': 0.1, 'memory_decay': 0.1, 'steps': 8},
    ]
